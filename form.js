(() => {
  ...
  // Keep AlphaFold3 controls in sync using the same display-based handling as Wisps.
  const af3FieldSelectors = (fieldName) => [
    `[name='${CONTEXT_PREFIX}[${fieldName}]']`,
    `[id='${CONTEXT_PREFIX}_${fieldName}']`,
    `[name='${fieldName}']`,
    `[id='${fieldName}']`,
    `[id$='_${fieldName}']`
  ];

  const findAf3Inputs = (fieldName, root) => {
    const scope = root || document;
    const inputs = [];
    af3FieldSelectors(fieldName).forEach((selector) => {
  ...
  const findAf3FieldContainer = (input) => {
  ...
  const setAf3FieldVisibility = (fieldName, visible, root) => {
    const inputs = findAf3Inputs(fieldName, root);
    const containers = [];

    inputs.forEach((input) => {
      const container = findAf3FieldContainer(input);
  ...
  const setAf3FieldRequired = (fieldName, required, root) => {
    findAf3Inputs(fieldName, root).forEach((input) => {
  ...
  const syncAf3WeightsVisibility = (form) => {
    if (!form) return;
    const methodInput = findAf3Inputs("af_method", form)[0];
    const showAf3Weights = methodInput && String(methodInput.value || "").trim() === "alphafold3";

    setAf3FieldVisibility("af3_weights", showAf3Weights, form);
    setAf3FieldVisibility("af3_warning", showAf3Weights, form);
    setAf3FieldRequired("af3_weights", showAf3Weights, form);
  };

  const attachAf3WeightsVisibility = () => {
    const form = Array.from(document.querySelectorAll("form")).find((candidate) =>
      findAf3Inputs("af_method", candidate).length > 0 && findAf3Inputs("af3_weights", candidate).length > 0
    );
    if (!form || form.dataset.oodAf3VisibilityAttached === "1") return false;

    form.dataset.oodAf3VisibilityAttached = "1";
    syncAf3WeightsVisibility(form);
    form.addEventListener("change", (event) => {
      if (findAf3Inputs("af_method", form).includes(event.target)) syncAf3WeightsVisibility(form);
    }, true);
    return true;
  };

  const keepTryingToAttachAf3WeightsVisibility = () => {
    if (attachAf3WeightsVisibility()) return;

    let tries = 0;
    const timer = window.setInterval(() => {
      tries += 1;
      if (attachAf3WeightsVisibility() || tries >= 50) window.clearInterval(timer);
    }, 200);
  };

  document.addEventListener("DOMContentLoaded", keepTryingToAttachAf3WeightsVisibility);
  document.addEventListener("turbo:load", keepTryingToAttachAf3WeightsVisibility);
  document.addEventListener("turbolinks:load", keepTryingToAttachAf3WeightsVisibility);
  keepTryingToAttachAf3WeightsVisibility();
